...
class AdamOptimizer(base.Base):
    ...
    def __init__(self, name):
        ...
        self._add_input("input_tensor", desc="A tensor to optimize")
        self._add_input("iteration_count", value=0)

        self._add_output("out", desc="A Tensor")

        self._add_param("iteration_count", value=0)
        self._add_param("learning_rate", value=0.001)
        self._add_param("beta1", value=0.9)
        self._add_param("beta2", value=0.999)
        self._add_param("epsilon", value=1e-08)
        self._add_param("use_locking", value=False)
        self._add_param("name", desc="Operation AdamOptimizer", value=name)
        ...
        self._add_optimize_param(fn_name="minimize", param_name="global_step")
        self._add_optimize_param(fn_name="minimize", param_name="var_list")
        self._add_optimize_param(fn_name="minimize", param_name="gate_gradients", value="GATE_OP")
        self._add_optimize_param(fn_name="minimize", param_name="aggregation_method")
        self._add_optimize_param(fn_name="minimize", param_name="colocate_gradients_with_ops", value=False)
        self._add_optimize_param(fn_name="minimize", param_name="name")
        self._add_optimize_param(fn_name="minimize", param_name="grad_loss")
    ...
    def _add_optimize_param(self, fn_name, param_name: str, value=None, desc=""):
        if fn_name == "minimize":
            self._add_param_to_obj(obj=self.minimize_params, name=param_name, value=value, desc=desc)

    def set_optimize_param(self, name, value):
        if self.optimize_function == "minimize":
            self._set_param_value_in_obj(obj=self.minimize_params, name=name, value=value)

    def get_optimize_param(self, name):
        if self.optimize_function == "minimize":
            return self._get_param_value_from_obj(self.minimize_params, name)
